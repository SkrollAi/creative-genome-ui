...
import { useTaggingFiltersStore } from "./use-tagging";

function deriveLabel(date_from: string, date_to: string) {
...
export function TaggingDatePicker() {
  const { filters, setFilters } = useTaggingFiltersStore();
  const [open, setOpen] = useState(false);
  const [customFrom, setCustomFrom] = useState("");
  const [customTo, setCustomTo] = useState("");
  const ref = useRef<HTMLDivElement>(null);

  const label = deriveLabel(filters.date_from, filters.date_to);
